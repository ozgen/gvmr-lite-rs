...
use crate::{
    ...
    infra::fs::{copy_dir_recursive, list_relative_files, make_executable_best_effort, walk_files},
...
#[tokio::test]
async fn render_report_xml_with_generate_returns_error_when_generate_script_is_missing() {
    let workdir = temp_test_dir("runner-missing-generate");

    let fmt = test_format(workdir.clone(), "txt", "text/plain");

    let err = render_report_xml_with_generate(&fmt, "<report />", &serde_json::Map::new(), 5, None)
        .await
        .unwrap_err();
...
#[tokio::test]
async fn render_report_xml_with_generate_returns_stdout_content() {
    let workdir = temp_test_dir("runner-stdout");
    ...
    let result = render_report_xml_with_generate(
        &fmt,
        "<report />",
        &serde_json::Map::new(),
        5,
    ...
    assert_eq!(result.content, b"hello from runner");
...
#[tokio::test]
async fn render_report_xml_with_generate_reads_output_file_when_stdout_is_empty() {
    let workdir = temp_test_dir("runner-output-file");
    ...
    let result =
        render_report_xml_with_generate(&fmt, "<report />", &serde_json::Map::new(), 5, None)
            .await
            .unwrap();
...
#[tokio::test]
async fn render_report_xml_with_generate_uses_fallback_filename_extension() {
    let workdir = temp_test_dir("runner-fallback-extension");

    fs::write(workdir.join("generate"), b"#!/bin/sh\nprintf 'hello'\n").unwrap();

    make_executable_best_effort(&workdir.join("generate"));

    let fmt = test_format(workdir.clone(), "", "text/plain");
    ...
    assert_eq!(result.filename, "report.bin");

    let _ = fs::remove_dir_all(workdir);
}

#[tokio::test]
async fn render_report_xml_with_generate_returns_no_output_error_when_command_produces_nothing() {
    let workdir = temp_test_dir("runner-no-output");
...
fn build_env_sets_expected_gvmr_variables_and_params() {
    ...
    assert_eq!(envs.get("GVMR_PARAM_FLAG").unwrap(), "true");
    assert_eq!(envs.get("GVMR_PARAM_EMPTY").unwrap(), "");
...
fn copy_format_assets_copies_nested_files() {
    let src = temp_test_dir("runner-copy-assets-src");
    let dst = temp_test_dir("runner-copy-assets-dst");
...
#[test]
fn snapshot_meta_records_existing_files() {
    let root = temp_test_dir("runner-snapshot-meta");

    fs::write(root.join("report.txt"), b"hello").unwrap();

    let snapshot = snapshot_meta(&root);

    assert!(snapshot.contains_key(&root.join("report.txt")));
...
fn pick_output_file_prefers_changed_file_with_matching_extension() {
    let root = temp_test_dir("runner-pick-output");
...
#[test]
fn pick_output_file_ignores_assets() {
    let root = temp_test_dir("runner-pick-output-assets");

    let asset = root.join("asset.txt");
    fs::write(&asset, b"asset").unwrap();

    let before = snapshot_meta(&root);
    let assets = HashSet::from([asset.clone()]);

    let picked = pick_output_file(&root, &before, &assets, Some("txt"));
...
#[test]
fn is_changed_returns_true_for_new_file() {
    let root = temp_test_dir("runner-is-changed-new");

    let before = snapshot_meta(&root);
    let path = root.join("new.txt");
    fs::write(&path, b"new").unwrap();

    assert!(is_changed(&path, &before));

    let _ = fs::remove_dir_all(root);
}

#[test]
fn is_changed_returns_false_for_unchanged_file() {
    let root = temp_test_dir("runner-is-changed-unchanged");
...
fn walk_files_recurses_nested_files() {
    let root = temp_test_dir("runner-walk-files");
    ...
    assert!(files.iter().any(|path| path.ends_with("nested.txt")));
...
fn list_relative_files_returns_relative_paths() {
    let root = temp_test_dir("runner-list-files");
...
fn make_executable_best_effort_adds_executable_bits() {
    ...
    let root = temp_test_dir("runner-make-executable");
...
fn copy_dir_recursive_copies_nested_files() {
    let src = temp_test_dir("runner-copy-dir-src");
    let dst = temp_test_dir("runner-copy-dir-dst");
...
#[test]
fn current_unix_timestamp_nanos_returns_non_zero_value() {
    assert!(current_unix_timestamp_nanos() > 0);
}
...
fn maybe_copy_debug_tmpdir_does_nothing_without_env_var() {
    ...
    let tmpdir = temp_test_dir("runner-debug-copy-source");
...
fn maybe_copy_debug_tmpdir_copies_tempdir_when_env_var_is_set() {
    ...
    let source = temp_test_dir("runner-debug-copy-source");
    let debug_root = temp_test_dir("runner-debug-copy-root");
