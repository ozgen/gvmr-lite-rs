pub const DISCARDED_REPORT_FORMAT_IDS: &[&str] = &[
    "77bd6c4a-1f62-11e1-abf0-406186ea4fc5",
    "ccc03c9e-d52c-4078-9b49-a7a6817a3a36",
    "5ceff8ba-1f62-11e1-ab9f-406186ea4fc5",
    "ce6ebf9b-3827-4e3c-bb92-5a8debace024",
    "37dad904-44d7-45f6-9b07-77089ea80689",
    "dc972eb1-0f37-4476-9d55-8203dbdccbec",
    "50c9950a-f326-11e4-800c-28d24461215b",
    "2a28a134-147c-11eb-8c6e-b05cda5b0faa",
    "1fb9036c-1439-11eb-9d5d-b05cda5b0faa",
];

pub const BUILT_IN_TYPST_TECHNICAL_ID: &str = "a9063afd-313e-434e-9008-9dfe3a22e00f";

pub const BUILT_IN_NATIVE_PDF_TECHNICAL_ID: &str = "a9063afd-313e-434e-9008-9dfe3a22e010";

pub const BUILT_IN_NATIVE_PDF_COMPLIANCE_ID: &str = "a9063afd-313e-434e-9008-9dfe3a22e011";

pub const DELTA_REPORT_FORMAT_IDS: &[&str] = &[BUILT_IN_NATIVE_PDF_TECHNICAL_ID];

pub const DELTA_AUDIT_REPORT_FORMAT_IDS: &[&str] = &[BUILT_IN_NATIVE_PDF_COMPLIANCE_ID];
